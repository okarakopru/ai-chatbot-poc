...
TASK:
- Understand which product the user means semantically.
- Respond with EXACT product name from list.
- If none matches, respond ONLY with: NONE
"""
...
USER QUERY: "{query}"
Return STRICT JSON with category and products.
...
- If type="product_info": mention only name, price, stock.
- If type="product_list": list items cleanly.
- If type="refund_policy": show policy only.
- If type="summary": show summary items.
- If type="info": output message.
- NEVER hallucinate.
- NEVER upsell.
...
def chat(req: ChatInput):
    ...
    # --------------------------------------------------------
    resolved_product = ai_resolve_product_name(user)
    if resolved_product:
        return {"reply": natural_format(user, tool_product_lookup(resolved_product))}
    ...
    if any(k in lower for k in summary_keywords):
        ...
        for name in items:
            for p in products.values():
                if p["name"] == name:
                    summary.append({"name": p["name"], "price": p["price"], "stock": p["stock"]})
        return {"reply": natural_format(user, {"type": "summary", "products": summary})}
    ...
    if ai_topic_recall(user):
        if memory["conversation_products"]:
            ...
            return {"reply": natural_format(user, tool_product_lookup(last))}
    ...
    # --------------------------------------------------------
    refund_keywords = ["refund", "return", "استرجاع", "ارجاع", "إرجاع", "مرتجعات", "رجوع", "اعادة"]
    ...
    if memory["last_intent"] == "order" and ai_order_id_missing(user):
        ...
        if product_name:
            return {"reply": natural_format(user, tool_product_lookup(product_name))}
